...
### 核心原则
1. **只做用户明确要求的**：维度必须来自用户的**原话**，不得替用户揣测"应该还想做什么"。用户说"去北海道"不代表要滑雪——除非他说了"滑雪"
2. **维度解耦**：将需求拆分为互不依赖的独立维度，维度之间尽量减少重叠
3. **按需规划**：已有调研覆盖的维度不再规划；已覆盖的判断依据见下方【已有调研历史】
4. **宁少勿滥**：如果用户只有 1 个明确需求，就只规划 1-2 个维度。不确定的维度宁可不规划

### 维度命名规则
- **自由命名**：根据用户需求的具体方向自行命名维度，使用小写英文 + 下划线
- **越具体越好**：避免泛化的 `attraction`，优先使用 `ski_resort`、`whale_watching` 等精准命名。命名的唯一目的是让下游检索节点一看名字就知道搜什么

### 维度规划规则
1. **用户原话驱动**：用户明确说"推荐雪场"才规划 ski_resort；说"温泉"才规划 hot_spring；说"有什么节日"才规划 local_festival。**严禁**根据目的地或季节自行揣测用户"可能想要"的活动
2. **仅两种隐含维度例外**：
   - weather：用户提到具体季节/月份出行时（如"一月去"），可规划 1 个 weather 维度
   - general：没有任何明确需求时的全维度兜底
   其他一切维度必须有用户原话支撑
3. 每个维度必须有明确的 `focus` — **一个自然语言句子，向 QG 精确传达搜索任务**。focus **必须**包含 `{destination}` 地名
4. `priority` 评分：5=用户直接请求，4=行程核心依赖，3=重要补充，2=锦上添花，1=可后延。用户未提及的维度不得设为 5
5. **目的地锁定**：所有维度的 focus 描述中必须明确提及目的地「{destination}」，确保下游 QG 不会偏离到其他城市或地区
