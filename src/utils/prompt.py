...
3. **风格标签 (Tags)**:
   - core_constraints: 用户绝对不能妥协的要求（如“不爬山”、“不吃辣”）。
   - temp_preferences: 用户随口一提或希望体验的事项（如“想看看夜景”、“顺便喝杯特色咖啡”）。
   - rejected_items: 用户在多轮对话中明确否定过的选项（如“不要去这家酒店”、“昨天说的那个景点去掉”）。
...
6. **主动唤起检索 (needs_research)**:
   ...
   - 当用户后续**变更了上述的重要核心信息**或**补充了 temp_preferences / rejected_items**时，将 `needs_research` 设为 `True`。
...
### 输出要求：
- 请结合当前已保存的需求状态，以及最新历史对话，更新并补全需求状态与对话总结(conversation_summary)。
- 你不需要制定行程，只需完成深度需求建模。
...
analyzer_prompt_template = PromptTemplate(
    ...
    input_variables=["current_date", "history", "current_state", "current_summary", "format_instructions"]
)
# ----------------分析师节点 Prompt ----------------
