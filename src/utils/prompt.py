...
1. **逻辑分流逻辑（Routing Logic）**：
   ...
   - **生成推荐**：当 `research_matches_current` 为 True 且调研已充分，但 `is_recommendation_complete` 为 False 时，导向 `recommender` 生成目的地/住宿/餐饮推荐
   - **检测用户选择**：当 `is_recommendation_complete` 为 True 且 `is_selection_made` 为 False 时（上一轮已生成推荐），分析用户最新消息：
     - 若用户表达了选择（如"选1号"、"第二家不错"、"目的地选A"），提取 `user_selections`（目的地/住宿/餐饮名称），设置 `is_selection_made=True`
     - 若用户不满意（"不满意"、"换一批"、"有没有更便宜的"），设置 `needs_reselect=True`，填入 `reselection_feedback`，路由到 `recommender` 重新推荐
     - 若用户放弃选择（"随便"、"都行"、"你定"），对应字段填 `"agent_choice"`
     - 若用户完全换了话题/目的地 → 正常新查询，路由到 `research_loop`
...
- 推荐是否完成 (is_recommendation_complete): {is_recommendation_complete}
  (由 Recommender 更新。True 表示目的地/住宿/餐饮推荐已生成)
...
- 已生成的推荐摘要: {recommendation_summary}
  (用户上一轮看到的推荐列表。当 is_recommendation_complete=True 且 is_selection_made=False 时，据此判断用户消息是否为选择/拒绝/重推)
...
manager_prompt_template = PromptTemplate(
    input_variables=["is_core_complete", "is_safe", "is_recommendation_complete", "is_plan_complete", "is_selection_made", "recommendation_summary", "hashes_count", "research_matches_current", "research_history", "history", "user_request", "trace_history", "format_instructions"],
...
### 你的任务
根据研究数据和用户画像，生成智能推荐方案：
1. **目的地推荐** (destinations): 1-3 个候选目的地，含推荐理由和评分
2. **住宿建议** (accommodations): 区域、类型、预算范围
3. **餐饮建议** (dining): 类型、特色菜品

### 评分标准
- 80-100: 高度匹配用户偏好，研究数据充分支撑
- 60-79: 基本匹配，部分信息需补充
- 40-59: 部分匹配但存在明显偏差
- 1-39: 勉强相关但不推荐

### 推荐策略 (strategy)
用一句话概括你的推荐思路，例如"优先推荐住宿集中在交通便利区域，兼顾预算和品质"。

### 运行规则
- 基于研究数据进行推荐，不要凭空编造
- 如果研究数据不足以支撑某类推荐，坦诚说明并降低评分
- 推荐理由必须具体，引用研究数据中的地点名、价格区间、特色等信息
...
recommender_prompt_template = PromptTemplate(
    input_variables=["current_time", "history", "user_request", "user_profile", "research_summary", "format_instructions"],
