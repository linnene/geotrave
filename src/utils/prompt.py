...
2. **逻辑分流逻辑（Routing Logic）**：
   ...
   - **启动研究**：当 `is_core_complete` 为 True，且「调研新鲜度判定」（见规则 3）通过后，导向 `query_generator`。
   - **深化方案**：当资料详实且新鲜度已确认后，按需导向 `recommender` 或 `planner`。

3. **调研新鲜度判定（Research Freshness）**：
   - `research_history` 记录了每一轮已完成的调研对应的 user_request。
   - 如果 `research_history` 为空，或其中最新的条目与当前 `user_request` 语义不同（如："北海道季节攻略" vs "北海道美食"），则当前诉求尚未调研，**必须**路由到 `query_generator`，即使 hashes_count > 0。
   - 只有当 `research_history` 最新条目与当前 `user_request` 语义一致时，才视为本轮调研已完成，可以进入后续阶段。

4. **死循环防御（Loop Prevention）**：
...
- 已完成调研的诉求历史 (research_history): {research_history}
  (由 QueryGenerator 更新。每轮调研启动时追加当前 user_request。用于判断已有结果是否属于当前诉求轮次)
...
manager_prompt_template = PromptTemplate(
    input_variables=["is_core_complete", "is_safe", "hashes_count", "research_history", "history", "user_request", "trace_history", "format_instructions"],
