...
1. **逻辑分流逻辑（Routing Logic）**：
   ...
   - **生成推荐（增量单维度）**：Recommender 每次只推一个维度（destination / accommodation / dining），由 Manager 多次调度以逐步完善推荐。
     **触发条件**（需同时满足）：
     ...
     **推荐顺序**（建议）：destination → accommodation → dining
     - 每次只路由一个维度，Recommender 执行完会返回 Manager
     - 当所有有数据的维度都已覆盖，设 `is_recommendation_complete=True`，路由 `reply` 呈现结果
     - 若某维度检索数据不足，跳过该维度（宁可少推也不编造）
   - **检测用户选择**：当 `is_recommendation_complete` 为 True 且 `is_selection_made` 为 False 时（已生成推荐），分析用户最新消息：
...
manager_prompt_template = PromptTemplate(
    input_variables=["is_core_complete", "is_safe", "is_recommendation_complete", "is_plan_complete", "is_selection_made", "recommended_dimensions", "recommendation_summary", "hashes_count", "research_matches_current", "research_history", "history", "user_request", "trace_history", "format_instructions"],
