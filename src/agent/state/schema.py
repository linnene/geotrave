...
class RetrievalMetadata(BaseModel):
    """存储在 ResearchManifest.verified_results 中的结果索引记录。

    注意: 此模型将在 Step 5 (Search 适配) 中重构。
    hash_key、source、relevance_score、payload 由工具写入，
    但目前未被任何下游消费者读取。
    """
...
class ResearchManifest(BaseModel):
    ...
    内部契约（仅 Research Loop 子图节点读写）:
        loop_state       — ResearchLoopInternal（详见该类）
        active_queries   — [已废弃] 将在 Step 5 迁移至 loop_state
        verified_results — [已废弃] 将在 Step 5 迁移至 loop_state

    废弃字段保留用于增量迁移期间的向后兼容，Search 节点适配完成后移除（Step 5）。
    """
    # --- 新字段 ---
    ...
    # --- 已废弃（正在迁移至 loop_state + research_hashes）---
    active_queries: List[SearchTask] = Field(
        default_factory=list,
        description="[DEPRECATED] Use loop_state; will be removed in Step 5"
    )
    verified_results: List[RetrievalMetadata] = Field(
        default_factory=list,
        description="[DEPRECATED] Use loop_state.all_passed_results; will be removed in Step 5"
    )

    # --- 保留 ---
    feedback_history: List[str] = Field(
        default_factory=list, description="Historical feedback entries from Critic"
    )
