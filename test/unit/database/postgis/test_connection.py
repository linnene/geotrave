...
async def test_get_pool_recreates_on_loop_mismatch():
    """
    Priority: P0
    Description: get_pool() closes the old pool (with try/except protection)
    and creates a new one when the current event loop differs from _pool_loop.
    """
    old_pool = AsyncMock()
    new_pool = AsyncMock()
    ...
    with patch("src.database.postgis.connection.asyncpg.create_pool", new=AsyncMock(return_value=new_pool)):
        ...
        old_pool.close.assert_awaited_once()  # 尝试关闭旧池（try/except 保护）
