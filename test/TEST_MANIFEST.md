...
| 5 | `test_tool_metadata_populated` — TOOL_METADATA 非空 | QueryGenerator 依赖元数据注入提示词，缺失将导致 LLM 不知道可用工具 |
| 6 | `test_tool_metadata_structure` — 元数据结构完整性 | name/description/parameters 缺失会导致提示词注入空值或格式异常 |
| 7 | `test_tool_dispatch_matches_metadata` — 注册一致性 | metadata 与 dispatch 不一致会导致 QueryGenerator 生成的工具名无对应 handler |
| 8 | `test_gateway_safe_routes_to_analyst` — 安全输入固定进 analyst | 拓扑核心变更：analyst 是 gateway 后的固定边，路由失败将跳过需求提取 |
| 9 | `test_gateway_unsafe_routes_to_reply` — 不安全输入直达 reply | 安全边界：malicious/chitchat 不得进入业务节点 |
| 10 | `test_manager_routes_query_generator` — Manager 调研路由 | 核心信息完整后必须能路由到 query_generator |
| 11 | `test_manager_routes_reply` — Manager 兜底路由 | 未知/异常情况下 fallback 到 reply |
| 12 | `test_spatial_search_returns_real_data` (INT) — 真实 POI 数据验证 | 实时数据库不可用将导致返回空结果 |
| 13 | `test_spatial_search_category_filter` (INT) — 类别过滤正确性 | 类别映射错误导致用户查询无结果 |
| 14 | `test_route_search_shortest_path` (INT) — 真实路网最短路径 | pgRouting 函数重载冲突已在 Phase 4 修复 |
| 15 | `test_route_search_isochrone` (INT) — 等时圈可达性 | 拓扑未构建或过时将导致零节点或错误距离 |
...
4. **图拓扑正确性**: 新增 `test_graph_routing.py` 守护 gateway → analyst 固定边、Manager 路由范围。若拓扑变更导致 analyst 被绕过，整个需求提取链路断裂。注意：测试中路由函数是 graph.py 闭包逻辑的副本，真实图编译需 async 环境。
