...
### 维度 4：集成与端到端（Integration & E2E）验证
- 目标：拟合真实用户对话负载，黑盒测试整体响应体系及 API 接口契约。
- **当前实现**：
  - **API 集成测试**：[test_api.py](../test/integration/test_api.py) 覆盖了 `/api/chat` 和 `/api/rag` 所有核心接口。
    - Chat 业务逻辑连通性（输入验证、状态返回）。
    - RAG 核心闭环（统计获取、批量插入、语义检索映射）。
    - 边界情况处理（非法文件格式、空输入校验）。
