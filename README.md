...
- **编排引擎**: [LangGraph](https://langchain-ai.github.io/langgraph/) - 强大的多 Agent 状态机循环（支持基于 **MemorySaver** 的持久化对话管理）。
- **混合大模型**:
  - 核心大脑 (Chat): **DeepSeek** (`deepseek-chat`) / **Google Gemini** (对话分析增强)
- **智能分析**: 具备 **历史记忆感知**、**动态日期推算** 与 **旅行风格自动标签标注** (穷游/自驾/特种兵等)。
- **混合大模型**:
...
- **对外接口**: [FastAPI](https://fastapi.tiangolo.com/) 构建高性能 RESTful 路由，支持 **session_id** 多用户会话管理。
